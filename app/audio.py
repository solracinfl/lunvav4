...
class AudioIO:
    ...
    def record_until_vad_end(
        ...
        discard_ms: int = 0,          # ignore initial audio (speaker bleed / device settle)
        ...
        discard_frames = max(0, discard_ms // frame_ms)
        ...
        try:
            while True:
                ...
                # Discard early audio to avoid capturing our own TTS and ALSA startup noise
                dbg_frame += 1  # reuse counter as a per-call frame index
                if discard_frames and dbg_frame <= discard_frames:
                    ring.clear()
                    captured.clear()
                    speech_started = False
                    speech_frames = 0
